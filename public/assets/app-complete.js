...
function initDashboard() {
  ...
  if(role === "STUDENT"){
    menu.innerHTML = `
      ...
      <div onclick="showInterviewSlots()">📅 Interview Slots</div>
      <div onclick="showMentorSlots()">👨‍🏫 Mentor Slots</div>
  ...
  if(role === "TPO"){
    menu.innerHTML = `
      ...
      <div onclick="showScheduleInterview()">📅 Schedule Interview</div>
  ...
  if(role === "ALUMNI"){
    menu.innerHTML = `
      <div onclick="showJobPosts()">💼 My Job Posts</div>
      <div onclick="showCreateJobPost()">➕ Create Job Post</div>
      <div onclick="showMentorManager()">👨‍🏫 Mentor Slots</div>
      <div onclick="showChangePassword()">🔐 Change Password</div>
    `;
    showJobPosts();
  }
  ...
  data.data.forEach(drive => {
    html += `
      <div class="card">
        ...
        <p><strong>Status:</strong> <span class="status-badge">${drive.status}</span></p>
      ...
      html += `
        <div class="card">
          ...
          <div class="card-actions">
            <button onclick="editProject(${project.id})" class="btn-small">Edit</button>
...
async function showInterviewSlots(){
  ...
  let html = "<h2>📅 Interview Slots</h2>";
  ...
  if(data.data && data.data.length > 0) {
    ...
    data.data.forEach(slot => {
      html += `
        <div class="card">
          <h4>Drive: ${slot.driveTitle}</h4>
          <p><strong>Time:</strong> ${new Date(slot.startTime).toLocaleString()}</p>
          <p><strong>Room:</strong> ${slot.room}</p>
          <button onclick="bookInterviewSlot(${slot.id})" class="btn-primary">Book Slot</button>
        </div>
      `;
    });
    html += "</div>";
...
async function showMentorSlots(){
  ...
  if(data.data && data.data.length > 0) {
    ...
    data.data.forEach(slot => {
      html += `
        <div class="card">
          <h4>${slot.mentorName}</h4>
          <p class="secondary">${slot.jobTitle}</p>
          <p><strong>Time:</strong> ${new Date(slot.startTime).toLocaleString()}</p>
          <button onclick="bookMentorSlot(${slot.id})" class="btn-primary">Book Slot</button>
        </div>
      `;
    });
    html += "</div>";
  ...
  if(data.data && data.data.length > 0) {
    ...
    data.data.forEach(job => {
      html += `
        <div class="card">
          <h4>${job.title}</h4>
          <p>From: <strong>${job.alumniName}</strong></p>
          <p class="secondary">${job.company}</p>
          <p>${job.description}</p>
...
async function showSkillGapAnalysis(){
  ...
  if(data.success && data.data) {
    ...
    html += `
      <div class="card">
        <h3>Your Analysis</h3>
        <p><strong>Current Skills:</strong> ${analysis.currentSkills || 'N/A'}</p>
        <p><strong>Required Skills:</strong> ${analysis.requiredSkills || 'N/A'}</p>
        <p><strong>Missing Skills:</strong> ${analysis.missingSkills || 'N/A'}</p>
        <p><strong>Recommendation:</strong> ${analysis.recommendation || 'Learn the missing skills to improve placement chances'}</p>
      </div>
    `;
  } else {
    html += "<p>No analysis available. Complete your profile first.</p>";
  }
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="card">
      <form id="createDriveForm" enctype="multipart/form-data">
        <label>Company</label>
        <input name="companyId" type="number" required placeholder="Company ID">
  ...
  data.data.forEach(drive => {
    html += `
      <div class="card">
        ...
        <button onclick="deleteDrive(${drive.id})" class="btn-danger">Delete</button>
      </div>
    `;
  });
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="card">
      <form id="scheduleForm">
        ...
        <input type="number" id="driveId" required>
...
async function scheduleInterview(e) {
  ...
  const token = localStorage.getItem("token");
  const formData = {
    driveId: parseInt(document.getElementById("driveId").value),
...
async function showMentorManager(){
  const token = localStorage.getItem("token");
  ...
  if(data.data && data.data.length > 0) {
    html += "<h3>Your Slots</h3><div class='grid'>";
    data.data.forEach(slot => {
      html += `
        <div class="card">
